...
class GCapiClient
{
  ...
  public:
    ...
    std::unordered_map<std::string, nlohmann::json> get_prices(std::vector<std::string> market_name_list, int num_ticks = 1,
                                                               long unsigned int from_ts = 0, long unsigned int to_ts = 0,
                                                               std::string price_type = "MID");

    std::unordered_map<std::string, nlohmann::json> get_ohlc(std::vector<std::string> market_name_list, std::string interval, int num_ticks = 1,
                                                             int span = 1, long unsigned int from_ts = 0, long unsigned int to_ts = 0);
